...
export function AIInsightsCard({ protocolId, peptideName, className }: AIInsightsCardProps) {
      ...
      <div className={cn(
        ...
        <div className="flex items-center justify-between mb-4">
          <div className="flex items-center gap-2">
            ...
            <span className="text-sm font-medium text-[var(--accent)]">AI Insights</span>
          </div>
        </div>
        <div className="space-y-4">
          <div className="space-y-2">
            <div className="h-3 w-20 bg-[var(--muted)] rounded animate-blur-reveal" />
          ...
          <div className="space-y-2">
            <div className="h-3 w-24 bg-[var(--muted)] rounded animate-blur-reveal" />
  ...
  if (error) {
    return (
      <div className={cn(
        'rounded-2xl p-5 border',
        'bg-gradient-to-br from-[var(--error-muted)] via-[var(--surface-2)] to-[var(--warning-muted)]',
        'border-[var(--error)]/20',
        className
      )}>
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <span className="text-sm font-medium text-[var(--error)]">Unable to load insights</span>
          </div>
          <button
         ...
         !data.assessment.suggestions[0].toLowerCase().includes('well-optimized') && (
          <div className="space-y-2">
            {data.assessment.suggestions.map((suggestion, i) => (
              <div key={i} className="flex items-start gap-2">
                <ArrowRight className="w-4 h-4 text-[var(--warning)] flex-shrink-0 mt-0.5" />
