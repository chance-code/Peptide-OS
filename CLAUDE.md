...
- 2026-04-23 — **Turso migration pattern**: `prisma migrate deploy` does NOT natively support libSQL URLs. The canonical deploy path is `scripts/deploy-refocus-to-turso.ts` — connects via `@libsql/client`, runs the migration SQL in a single write transaction, and records it in `_prisma_migrations`. Pattern should repeat for every future schema change.
- 2026-04-23 — **Migration-history drift fixed** via baseline reset: 4 prior migrations had `db push`-era divergence (21 orphan tables never captured). Collapsed into a single `00000000000000_baseline` migration, marked applied via `prisma migrate resolve --applied` so neither dev.db nor Turso re-runs it. New migrations now generate cleanly. If this happens again: use the same pattern — dump current DB schema, baseline it, mark applied, generate new migrations forward.
- 2026-04-23 — **DoseSchedule is now authoritative** for "what's due today." `/api/today` + `/api/cron/reminders` read from it; `src/lib/schedule-materializer.ts` populates it on protocol/cycle/titration writes + nightly cron. On-the-fly `resolveDose()` is a fallback only when the cache is cold. Cycle/titration changes MUST call `materializeScheduleForProtocol` in the same transaction or the cache desyncs.
- 2026-04-23 — **Reminders were silently broken pre-refocus**. The cron counted `DoseLog` rows with `status='pending'`, but those rows were never created up-front. A user with 5 active protocols and 0 DoseLog rows got zero reminders. Fix in `src/lib/pending-doses.ts#countPendingDoseSlots` — derives expected from `resolveDose()` minus completed. Keep that invariant.
